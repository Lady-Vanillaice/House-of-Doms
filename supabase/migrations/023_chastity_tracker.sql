-- Live Keuschhaltungs-/Chastity-Tracker fuer einvernehmliche House-Dynamiken.
-- Dokumentiert Status und Check-ins; speichert bewusst keine physischen Schluessel-/Lock-Codes.
...
create policy "chastity participants read records" on public.chastity_records
for select to authenticated using (auth.uid() in (dom_id,sub_id));

drop policy if exists "chastity participants read checkins" on public.chastity_checkins;
create policy "chastity participants read checkins" on public.chastity_checkins
for select to authenticated using (
  exists(select 1 from public.chastity_records r where r.id=record_id and auth.uid() in (r.dom_id,r.sub_id))
);
...
returns table(
  ...
  device_label text, started_at timestamptz, planned_review_at timestamptz,
  ended_at timestamptz, status text, notes text, end_reason text,
...
language sql security definer set search_path=public stable as $$
  select r.id,r.house_id,r.dom_id,r.sub_id,
    ...
    r.device_label,r.started_at,r.planned_review_at,r.ended_at,r.status,r.notes,r.end_reason,r.emergency_release_available
...
language sql security definer set search_path=public stable as $$
  ...
  from public.chastity_checkins c
  join public.chastity_records r on r.id=c.record_id
  join public.profiles p on p.id=c.submitted_by
  where c.record_id=p_record_id and auth.uid() in (r.dom_id,r.sub_id)
...
begin
  ...
  values(v_house,auth.uid(),p_sub_id,coalesce(nullif(trim(p_device_label),''),'Keuschheitskaefig'),p_planned_review_at,nullif(trim(coalesce(p_notes,'')),''))
...
begin
  ...
  insert into public.chastity_checkins(record_id,submitted_by,comfort_status,note)
  values(p_record_id,auth.uid(),v_status,nullif(trim(coalesce(p_note,'')),'')) returning id into v_id;
  if v_status='emergency' and r.status<>'ended' then
    update public.chastity_records set status='ended',ended_at=now(),end_reason='Notfall-Entriegelung / sofort beendet',updated_at=now() where id=p_record_id;
  end if;
  return v_id;
end; $$;
