...
export default function ExperienceLayer() {
  ...
  return <>
    ...
    {inside && panel && <aside className="gadgetPanel"><button className="gadgetClose" onClick={() => setPanel(null)}>×</button><span className="gadgetLabel">{panel === "key" ? t.key : panel === "alerts" ? t.alerts : t.quick}</span>{panel === "key" && <><div className="digitalHouseKey"><small>HOUSE OF DOMS</small><strong>HOD-0001</strong><span>OBSIDIAN · ACTIVE</span></div><button className="gadgetAction" onClick={copyKey}>{copied ? t.copied : t.copy}</button></>}{panel === "alerts" && <div className="gadgetFeed"><article><b>{t.planned}</b><span>08. Aug. · 18:00</span></article><article><b>{t.session}</b><span>10. Aug. · 12:00</span></article><a href="/benachrichtigungen">{t.notifications} →</a></div>}{panel === "quick" && <div className="quickLinks"><a href="/discover">{t.discover}</a><a href="/houses">{t.directory}</a><a href="/studios">{t.studios}</a><a href="/sessions">{t.sessions}</a><a href="/benachrichtigungen">{t.notifications}</a><a href="/kalender">{t.calendar}</a><a href="/bewerbungen">{t.applications}</a><a href="/journal">{t.journal}</a><a href="/house-einstellungen">{t.settings}</a></div>}</aside>}
  </>;
}
