...
const copy = {
  de: { opening: "DIE TÜR ÖFFNET SICH", welcome: "WILLKOMMEN ZU HAUSE", key: "HOUSE KEY", alerts: "MITTEILUNGEN", quick: "SCHNELLZUGRIFF", copied: "Kopiert", copy: "Schlüssel kopieren", planned: "1 Aufgabe geplant", session: "Nächste Session", directory:"House-Verzeichnis", calendar: "Kalender", applications: "Bewerbungen", journal: "Journal", settings: "Einstellungen", sound: "Atmosphäre", focus: "Fokus" },
  en: { opening: "THE DOOR IS OPENING", welcome: "WELCOME HOME", key: "HOUSE KEY", alerts: "NOTIFICATIONS", quick: "QUICK ACCESS", copied: "Copied", copy: "Copy key", planned: "1 task planned", session: "Next session", directory:"House Directory", calendar: "Calendar", applications: "Applications", journal: "Journal", settings: "Settings", sound: "Atmosphere", focus: "Focus" }
};
...
export default function ExperienceLayer() {
  ...
  return <>
    ...
    {inside && panel && <aside className="gadgetPanel"><button className="gadgetClose" onClick={() => setPanel(null)}>×</button><span className="gadgetLabel">{panel === "key" ? t.key : panel === "alerts" ? t.alerts : t.quick}</span>{panel === "key" && <><div className="digitalHouseKey"><small>HOUSE OF DOMS</small><strong>HOD-0001</strong><span>OBSIDIAN · ACTIVE</span></div><button className="gadgetAction" onClick={copyKey}>{copied ? t.copied : t.copy}</button></>}{panel === "alerts" && <div className="gadgetFeed"><article><b>{t.planned}</b><span>08. Aug. · 18:00</span></article><article><b>{t.session}</b><span>10. Aug. · 12:00</span></article></div>}{panel === "quick" && <div className="quickLinks"><a href="/houses">{t.directory}</a><a href="/kalender">{t.calendar}</a><a href="/bewerbungen">{t.applications}</a><a href="/journal">{t.journal}</a><a href="/house-einstellungen">{t.settings}</a></div>}</aside>}
  </>;
}
