...
export default function ChamberPage(){
 ...
 const loadContacts=useCallback(async()=>{const supabase=createClient();const {data:auth}=await supabase.auth.getUser();if(!auth.user){location.href="/anmelden";return;}setMe(auth.user.id);const {data,error}=await supabase.rpc("get_chamber_contacts");if(error){setNotice(`Supabase: ${error.message}`);setLoading(false);return;}const list=(data||[]) as Contact[];setContacts(list);setActive(v=>v||list[0]?.user_id||"");setLoading(false);},[]);
 ...
 useEffect(()=>{void loadContacts();},[loadContacts]); useEffect(()=>{if(active)void loadMessages(active);else setMessages([]);},[active,loadMessages]);
 ...
 async function send(e:FormEvent){e.preventDefault();if(!active||(!draft.trim()&&!pendingFile))return;setSending(true);setNotice("");const supabase=createClient();let path:string|null=null;let type:string|null=null;try{
   ...
   setDraft("");setPendingFile(null);setShowNew(false);await loadMessages(active);await loadContacts();
