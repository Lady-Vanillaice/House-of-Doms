...
export async function POST(request:Request){
 ...
 if(!response.ok){\n  const raw=await response.text();\n  let detail="OpenAI hat die Anfrage abgelehnt.";\n  try{const parsed=JSON.parse(raw) as {error?:{message?:string;code?:string;type?:string}};const e=parsed.error;detail=[e?.message,e?.code?`Code: ${e.code}`:"",e?.type?`Typ: ${e.type}`:""].filter(Boolean).join(" · ")||detail;}catch{detail=raw.slice(0,500)||detail;}\n  return NextResponse.json({error:"KI-Anfrage fehlgeschlagen.",detail:detail.slice(0,700)},{status:502});\n }
