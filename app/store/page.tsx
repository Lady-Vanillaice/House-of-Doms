...
export default function StorePage(){
  ...
  return <main className="storePage">
    ...
    {role==="visitor" ? <>
      ...
      <div className="adminPanel"><span className="eyebrow">NEUER CONTENT</span><h2>Inhalt kategorisieren</h2><label>Titel<input value={title} onChange={e=>setTitle(e.target.value)}/></label><label>Preis in €<input type="number" min="0" step="1" value={price} onChange={e=>setPrice(e.target.value)}/></label><label>Typ<select value={type} onChange={e=>setType(e.target.value as ProductType)}><option value="digital">Digitaler Content</option><option value="package">Paket / Workshop</option><option value="session">Session-Angebot</option></select></label><label>Sichtbarkeit<select value={visibility} onChange={e=>setVisibility(e.target.value as Product["visibility"])}><option value="public">Öffentlich</option><option value="members">Nur Mitglieder</option><option value="house">Nur House-Verbindungen</option></select></label><div className="categoryField"><strong>Kategorien auswählen</strong><small>Mehrfachauswahl möglich – diese Kategorien steuern später den persönlichen Feed.</small><div className="categoryPicker compact">{categories.map(c=><button type="button" key={c} className={draftCategories.includes(c)?"selected":""} onClick={()=>toggleDraftCategory(c)}>{draftCategories.includes(c)?"✓ ":"+ "}{c}</button>)}</div></div><button className="primary" onClick={addDraft}>Als Entwurf anlegen</button><p className="hint">Ein Inhalt kann mehreren Kinks gleichzeitig zugeordnet werden, z. B. „Latex + Fußfetisch + Video“.</p></div>
      <div className="adminPanel inventory"><span className="eyebrow">INVENTAR</span><h2>Content verwalten</h2>{items.map(p=><article key={p.id}><div><strong>{p.title}</strong><span>{euro(p.price)} · {p.categories.join(" · ")}</span></div><em className={p.status}>{p.status==="published"?"Veröffentlicht":p.status==="draft"?"Entwurf":"Archiviert"}</em><button onClick={()=>cycle(p.id)}>{p.status==="draft"?"Veröffentlichen":p.status==="published"?"Archivieren":"Zurück zu Entwurf"}</button></article>)}</div>
    </section>}

    <section className="storeNotice"><strong>Personalisierung ohne Schubladen</strong><p>Interessen sind unabhängig von Dom/Sub-Rollen. Jeder Member kann frei auswählen, welche Kinks und Content-Arten im persönlichen Feed erscheinen sollen.</p></section>
  </main>;
}
