...
type Session = { id:string; sub:string; dom:string; date:string; from:string; to:string; studio:string; status:Status; note:string; internalNote:string };
...
function SessionsContent(){
  ...
  useEffect(()=>{if(creator)return;async function load(){try{const supabase=createClient();const {data:auth}=await supabase.auth.getUser();if(!auth.user){window.location.href="/anmelden?next=/sessions";return}const {data,error:e}=await supabase.from("session_requests").select("*").eq("creator_id",auth.user.id).order("requested_date",{ascending:true});if(e)throw e;const mapped=(data||[]).map((x:any)=>({id:x.id,sub:"Member",dom:x.creator_name,date:x.requested_date,from:String(x.requested_time).slice(0,5),to:"",studio:x.session_type,status:x.status as Status,note:x.note||"",internalNote:""}));setSessions(mapped);if(mapped[0])setSelected(mapped[0].id)}catch(e){setError(e instanceof Error?e.message:"Sessions konnten nicht geladen werden.")}finally{setLoading(false)}}load()},[creator]);
  async function update(patch:Partial<Session>){if(!current)return;setSessions(list=>list.map(s=>s.id===current.id?{...s,...patch}:s));const db:any={updated_at:new Date().toISOString()};if(patch.status)db.status=patch.status;if(patch.date)db.requested_date=patch.date;if(patch.from)db.requested_time=patch.from;const supabase=createClient();const {error:e}=await supabase.from("session_requests").update(db).eq("id",current.id);if(e)setError(e.message)}
  ...
  return <main className="managementPage">
    ...
    {error&&<p className="authMessage">{error}</p>}{loading?<p className="notice">Buchungsanfragen werden geladen …</p>:null}
