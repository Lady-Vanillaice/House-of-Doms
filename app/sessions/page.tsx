...
function SessionsContent(){
  ...
  useEffect(()=>{if(creator)return;async function load(){try{const supabase=createClient();const {data:auth}=await supabase.auth.getUser();if(!auth.user){window.location.href="/anmelden?next=/sessions";return}const {data:profile}=await supabase.from("profiles").select("display_name").eq("id",auth.user.id).maybeSingle();const names=Array.from(new Set([profile?.display_name,auth.user.user_metadata?.display_name,auth.user.user_metadata?.name].map(x=>String(x||"").trim()).filter(Boolean)));let query=supabase.from("session_requests").select("*").order("requested_date",{ascending:true});if(names.length)query=query.in("creator_name",names);const {data,error:e}=await query;if(e)throw e;const mapped=(data||[]).map((x:any)=>({id:x.id,sub:"Member",dom:x.creator_name,date:x.requested_date,from:String(x.requested_time).slice(0,5),to:"",studio:x.session_type,status:x.status as Status,note:x.note||"",internalNote:""}));setSessions(mapped);if(mapped[0])setSelected(mapped[0].id)}catch(e){setError(e instanceof Error?e.message:"Sessions konnten nicht geladen werden.")}finally{setLoading(false)}}load()},[creator]);
